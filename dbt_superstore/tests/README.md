...
### Run This Singular Test
```bash
dbt test --select assert_profit_not_greater_than_sales
```

---

## Singular Tests in This Directory

### 1. assert_profit_not_greater_than_sales.sql

**What it tests:** Profit should never exceed sales amount

**Business Rule:** It's impossible for profit to be greater than the sales amount

**Returns:** Records where `profit > sales` (failing records)

**Expected Result:** 0 records (test passes)
...
Then run:
```bash
dbt test --select assert_ship_date_after_order_date
```
